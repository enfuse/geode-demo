...
curl -H "Accept: application/json" -X POST "http://localhost:9393/streams/definitions" -i \
    ...
    -d "definition=file --server.port=20001 --mode=lines --file.directory=/tmp/foo/ |postgres-processor --spring.datasource.driver-class-name=org.postgresql.Driver --spring.datasource.password=app --spring.datasource.url=jdbc:postgresql://postgres.localhost:5432/app --spring.datasource.username=app|log"
